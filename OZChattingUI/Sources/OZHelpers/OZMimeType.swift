...
public struct OZMimeType {
  ...
  /// Number of bytes required for `OZMimeType` to be able to check if the
  /// given bytes match with its mime type magic number specifications.
  fileprivate let bytesCount: Int

  /// A function to check if the bytes match the `OZMimeType` specifications.
  fileprivate let matches: ([UInt8], OZSwime) -> Bool

  ///  Check if the given bytes matches with `OZMimeType`
  ...
  ///
  ///  - returns: Bool
  public func matches(bytes: [UInt8], swime: OZSwime) -> Bool {
    return bytes.count >= bytesCount && matches(bytes, swime)
  }

  /// List of all supported `OZMimeType`s
  public static let all: [OZMimeType] = [
    OZMimeType(
      mime: "image/jpeg",
      ext: "jpg",
      type: .jpg,
